...
export default class Telegram extends Service {
      ...
      public async entry(body){
          ...
          const {first_name,last_name} = body.message.chat;
          ...
          if(resA.sqlstatus == 'Failed'){
              return `@${first_name} ${last_name} Your code is invalid \n\n 您的邀请码无效`; 
          }
          return `@${first_name} ${last_name} Your code:${text} is activated successfully,send shared link to your friend right away to get your bonus. \n\n 你的验证码：${text}，已激活成功！立刻发送分享链接给朋友获得空投奖励！\n\n Your share link （你的分享链接): http://wisdomcoin.pro/?code=${text}`
      }
